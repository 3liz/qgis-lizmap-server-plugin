...
class TestServerCore(unittest.TestCase):
    def test_config_value_to_boolean(self):
        ...
        true_values = ["True", "true", "Yes", "yes", "T", "t", "1", 1, 1.0, [""], {"a": "b"}]
        for v in true_values:
            self.assertTrue(to_bool(v), v)

        false_values = ["False", "false", "NO", "no", "F", "f", "0", "foobar", 0, 0.0, None, [], {}]
    ...
    def test_get_lizmap_layers_config(self):
        ...
        cfg_layers = get_lizmap_layers_config(
    ...
    def test_get_lizmap_layer_login_filter(self):
        ...
        self.assertIsNone(get_lizmap_layer_login_filter({"loginFilteredLayers": "bar"}, "lines-geojson"))
        self.assertIsNone(get_lizmap_layer_login_filter({"loginFilteredLayers": {}}, "lines-geojson"))
        self.assertIsNone(
            get_lizmap_layer_login_filter({"loginFilteredLayers": {"lines-geojson": {}}}, "lines-geojson"),
        )
        self.assertIsNone(
            get_lizmap_layer_login_filter(
                {
